...
func Test_returnsForLuckySenselessPhrase(t *testing.T) {
	...
	if !respond || !utils.Contains(brain.GetSenselessPhrases(), response) {
...
func Test_huefyText(t *testing.T) {
	...
	data := map[string]string{
		"":                     "",
		"джо":                  "джо",
		"что-то заблокировать": "что-то хуовать", // todo improve
		"двуличие":             "хуичие",
		"Джокер":               "хуокер",
		"слышно":               "хуишно",
		"чекнуть":              "хуекнуть",
		...
		"Потому что надо было не шифры ваши писать": "хуому что хуядо хуило не хуифры хуяши хуисать",
	}
	for k, expected := range data {
...
func Test_huefyLastWord(t *testing.T) {
	brain := NewBrain(NewMemory(), true)
	data := map[string]string{
		"":                     "",
		"джо":                  "джо",
