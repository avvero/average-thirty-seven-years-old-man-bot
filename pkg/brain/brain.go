...
func (brain *Brain) Decision(chatId int64, user string, text string) (respond bool, response string, toxicityScore float64) {
	...
	respond, response = with(strings.ToLower(strings.TrimSpace(text))).
		...
		//
		when(is(brain.randomFactor), random(10), length(30)).then(&OpenApiIntention{brain: brain, text: user + " говорит \"" + text + "\". Проверь на грамотность, предложи исправления. В конце вставь ироничный дружеский комментарий в стиле Джимми Карра"}).
