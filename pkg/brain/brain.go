...
func (brain *Brain) Decision(chatId int64, user string, text string) (respond bool, response string, toxicityScore float64) {
	...
	respond, response = with(strings.ToLower(strings.TrimSpace(text))).
		...
		"Ответить на реплику в стиле шаблона ниже, опираясь на глагол из оригинальной реплики. Верни только ответ, реплику повторять не нужно.\n" +
		"---\n" +
		"Шаблон: \n" +
		...
		when(is(brain.randomFactor), random(100), length(300)).then(&OpenApiIntention{brain: brain,
